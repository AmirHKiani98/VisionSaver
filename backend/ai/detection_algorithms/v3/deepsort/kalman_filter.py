...
class KalmanFilter(object):
    ...
    def initiate(self, measurement):
        ...
        """
        m = torch.as_tensor(measurement, dtype=torch.float32, device=DEVICE)
        mean_pos = m
    ...
    def update(self, mean, covariance, measurement):
        ...
        """
        meas = torch.as_tensor(measurement, dtype=torch.float32, device=DEVICE)
        proj_mean, proj_cov = self.project(mean, covariance)

        # Torch equivalent of cho_solve
        L = torch.linalg.cholesky(proj_cov)
        K = torch.cholesky_solve((covariance @ self._update_mat.T).T, L, upper=False).T  # Kalman gain
    ...
    def gating_distance(self, mean, covariance, measurements,
        ...
        if only_position:
            ...
            d = torch.as_tensor(measurements[:, :2], dtype=torch.float32, device=DEVICE) - proj_mean
        else:
            d = torch.as_tensor(measurements, dtype=torch.float32, device=DEVICE) - proj_mean

        L = torch.linalg.cholesky(proj_cov)
        # Solve L z = d^T  (triangular solve); returns z [dim x N]
        z = torch.linalg.solve(L, d.T)
        return (z*z).sum(dim=0).detach().cpu().numpy()
